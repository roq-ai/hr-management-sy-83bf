...
export function PerformanceEvaluationListPage(props: PerformanceEvaluationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPerformanceEvaluations({
        relations: [
          'employee_performance_evaluation_employee_idToemployee',
          'employee_performance_evaluation_evaluator_idToemployee',
        ],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['comments.contains', 'period.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'employee_performance_evaluation_employee_idToemployee',
          header: 'Employee Performance Evaluation Employee Id Toemployee',
          accessorKey: 'employee_performance_evaluation_employee_idToemployee',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/employees/view/${record.employee_performance_evaluation_employee_idToemployee?.id}`}
            >
              {record.employee_performance_evaluation_employee_idToemployee?.job_title}
            </Link>
          ),
        }
      : null,
    hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'employee_performance_evaluation_evaluator_idToemployee',
          header: 'Employee Performance Evaluation Evaluator Id Toemployee',
          accessorKey: 'employee_performance_evaluation_evaluator_idToemployee',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/employees/view/${record.employee_performance_evaluation_evaluator_idToemployee?.id}`}
            >
              {record.employee_performance_evaluation_evaluator_idToemployee?.job_title}
            </Link>
          ),
        }
      : null,
